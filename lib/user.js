module.exports = function (restClient, options) {
  let module = {};
  let url = options.urlPrefix || 'user/';
  ...
  module.orderHistory = function (customerToken, page, pageSize) {
    url += `orderHistory?token=${customerToken}&curPage=${page}&pageSize=${pageSize}`
  ...
  module.setNewPassword = function (passwordData) {
    url += `setNewPassword`;

    return restClient.post(url, passwordData).then((data) => {
      return getResponse(data)
    })
  }
  module.resendAccountConfirmation = function (email) {
    url += `confirmation`;

    return restClient.post(url, {email: email}).then((data) => {
      return getResponse(data)
    })
  }
  return module;
